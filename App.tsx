...
const App: React.FC = () => {
  ...
  }, [chatSessions, theme, lang]);

  const handleLogin = (user: User) => setAuthState({ user, isAuthenticated: true });
  const handleLogout = () => {
  ...
  if (!authState.isAuthenticated) {
    return <Auth onLogin={handleLogin} theme={theme} lang={lang} />;
  }

  return (
    <HashRouter>
      <div className={`flex flex-col md:flex-row h-screen overflow-hidden ${theme === 'dark' ? 'bg-[#050505] text-slate-100' : 'bg-slate-50 text-slate-900'} transition-colors duration-300`}>
        <Sidebar onLogout={handleLogout} theme={theme} lang={lang} />
        ...
        <main className="flex-1 flex flex-col h-full relative overflow-hidden pb-16 md:pb-0">
          <Routes>
            ...
            <Route path="/chats" element={<Chats sessions={chatSessions} setSessions={setChatSessions} onSelectChat={(id) => setActiveChatId(id)} theme={theme} lang={lang} />} />
            <Route path="/help" element={<HelpCentre theme={theme} lang={lang} />} />
            <Route path="/profile" element={<Profile user={authState.user} onLogout={handleLogout} theme={theme} lang={lang} />} />
    ...
    <>
      <aside className={`hidden md:flex w-64 glass border-r ${theme === 'dark' ? 'border-slate-800' : 'border-slate-200'} flex-col z-20`}>
        <div className="p-6">
          ...
          <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mt-1">CeloraAI Powered</p>
        </div>
        ...
        <div className="p-4 mt-auto">
          <button onClick={() => { onLogout(); navigate('/'); }} className={`w-full flex items-center justify-center space-x-2 px-4 py-3 rounded-xl transition-all border ${theme === 'dark' ? 'bg-slate-900 border-slate-800 text-slate-400 hover:text-red-400' : 'bg-slate-100 border-slate-200 text-slate-500 hover:text-red-500'}`}>
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 h-16 glass border-t ${theme === 'dark' ? 'border-slate-800 bg-[#050505]/95' : 'border-slate-200 bg-white/95'} z-50 flex items-center justify-around px-2`}>
        {navItems.map((item) => (
          <Link key={item.path} to={item.path} className={`flex flex-col items-center justify-center w-full h-full ${location.pathname === item.path ? 'text-indigo-500' : 'text-slate-500'}`}>
            ...
            <span className="text-[9px] font-medium mt-1 truncate px-1">{item.label}</span>
          </Link>
        ))}
      </nav>
    </>
